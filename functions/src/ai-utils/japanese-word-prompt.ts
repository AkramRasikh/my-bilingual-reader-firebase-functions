...
const baseFormPrompt = `${sharedWordProperties.baseForm}: Give me the base form of the word provided. For example the base form (infinite form) of 泳げます is 泳ぐ. If possible, also do this when multiple verbs are used together for example 勉強して遊びたい => 勉強して遊ぶ. However, if this detracts from the meaning then leave it as the surface level provided i.e. 勉強して遊びたい. If it is a phrase then leave it as it is for example じゃあいってきます can be left as it is.`;
const notePrompt = `${sharedWordProperties.notes}: This is a field to mention anything nuanced happening in the language that may be missed. For example 彷徨う is pronounced ‘samayou’ but 彷徨 is pronounced ‘hōkō’. Anything that may be relevant to the learner given the word, it’s use in the context provided and how the word maybe used in another context would be useful.`;
const transliterationPrompt = `${sharedWordProperties.transliteration}: is the transliteration of the baseForm.`;
const phoneticPrompt = `${sharedWordProperties.phonetic}: is the romanji/katakana of the ${sharedWordProperties.baseForm}.`;

const japaneseformatTranslationPrompt = (japaneseWord, context) => {
  return `
...
export { japaneseformatTranslationPrompt };
